import { type ReactNode, useState } from 'react';
import Button from '../ui/Button';

interface InputGroupProps {
    children?: ReactNode;
    className?: string;
    placeholder?: string;
    inputIcon?: 'search' | null;
}

const InputGroup = ({
    children,
    className = '',
    placeholder = "",
    inputIcon = null
}: InputGroupProps) => {
    const [isFocused, setIsFocused] = useState(false);

    const inputIcons = {
        search: (
            <svg
                className="w-4 h-4"
                fill="currentColor"
                viewBox="0 0 512 512"
                xmlns="http://www.w3.org/2000/svg"
            >
                <path d="M416 208c0 45.9-14.9 88.3-40 122.7L502.6 457.4c12.5 12.5 12.5 32.8 0 45.3s-32.8 12.5-45.3 0L330.7 376c-34.4 25.2-76.8 40-122.7 40C93.1 416 0 322.9 0 208S93.1 0 208 0S416 93.1 416 208zM208 352a144 144 0 1 0 0-288 144 144 0 1 0 0 288z" />
            </svg>
        ),
    };

    return (

        <div className={`${isFocused ? 'animate-custom-shadow' : 'no-shadow'} ${inputIcon ? 'pr-0' : 'pr-3'} bg-white flex items-center gap-4 rounded-2xl border border-gray-200 ${className}`}>

            <div className='absolute ml-6 opacity-40'>
                {inputIcon && inputIcons[inputIcon]}
            </div>

            <input
                type="text"
                className={` ${inputIcon ? '' : 'md:w-[380px]'} !border-none !outline-none !ring-0 ${inputIcon ? 'pl-14 py-0' : 'pl-8 py-6'}`}
                placeholder={placeholder}
                onFocus={() => setIsFocused(true)}
                onBlur={() => setIsFocused(false)}
            />
            <Button variant="fill" rounded="normal">
                {children}
            </Button>
        </div>
    );
};

export default InputGroup;